...
class SupervisedGraphSage(nn.Module):
    ...
    def loss(self, nodes, labels, sat_problem, is_train):
        ...
        signed_variable_mask_transpose, function_mask = SatLossEvaluator.compute_masks(sat_problem._graph_map,
                                                                                       sat_problem._batch_variable_map,
                                                                                       sat_problem._batch_function_map,
                                                                                       sat_problem._edge_feature,
                                                                                       self._device)
        ...
        return self.lent(clause_values, sat_problem._function_num)
...
def train_batch(solver_base, data_loader, total_loss, rep, epoch, model_list, device, batch_replication,
        hidden_dimension, feature_dim, train_graph_recurrence_num, train_outer_recurrence_num, use_cuda = True,
        is_train = True, randomized = True):
    np.random.seed(1)
    random.seed(1)
    '''优化参数列表'''
    ...
    for (j, data) in enumerate(data_loader, 1):
        ...
        for seg in range(segment_num):
            ...
            for (k, model) in enumerate(model_list):
                ...
                print('{:s}: segment: {:d}, error={:s}|'.format(_module(model)._name, seg,
                                                                np.array_str(errors[:, k].flatten())))
