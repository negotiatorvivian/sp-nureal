...
class Encoder(nn.Module):
    def __init__(self, features, feature_dim, embed_dim, adj_lists, node_adj_lists, aggregator, num_sample = 10,
        ...
        self.gru = nn.GRUCell(self.feat_dim, gru_hidden_dim, bias = True) if gru else False
        # self.weight = nn.Parameter(
        #         torch.FloatTensor(embed_dim, gru_hidden_dim if gru else 2 * self.feat_dim))
        self.weight = nn.Parameter(torch.FloatTensor(self.embed_dim, gru_hidden_dim if gru else 2 * self.feat_dim))
