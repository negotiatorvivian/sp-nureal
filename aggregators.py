...
class MeanAggregator(nn.Module):
    ...
    def forward(self, nodes, to_neighs, num_sample = 10):
        ...
        embed_matrix = self.features(torch.LongTensor(unique_nodes_list, device = self._device))
...
class SurveyAggregator(nn.Module):
    def __init__(self, device, hidden_dimension, embed_dimension, edge_dimension, meta_data_dimension,
                 include_adaptors = False, pi = 0.0):
        ...
        self.norm_var_layer = nn.Sequential(
            nn.BatchNorm1d(self._embed_dim),
            nn.Linear(self._embed_dim, self._function_message_dim)
        )
    ...
    def forward(self, init_state, decimator_state, sat_problem, is_training, pure_sp = True):
        ...
        '''求出正影响的变量'''
        pos = torch.mm(p_variable_mask, decimator_function_state)  # decimator_function_state[edge,1]
        ...
        if self._previous_function_state is not None and not is_training:
            '''比较两次的变化'''
            function_diff = (self._previous_function_state - function_state[:, 0]).abs().unsqueeze(1)
        ...
        if not pure_sp:
            decimator_state = self.decimate(decimator_state, (variable_state, function_state), sat_problem)
